...
let m = {
  ...
  condition() {
    this.$http.post('appServiceFarmhouse/getMapParam',{rest: cityRestfulDictionary}).then(res => {
      // 区域
      ...
      this.farmhouseRentPrice = PriceArray
      this.farmhouseRentalArea = AreaArray
      this.farmhouseRentalLife = LifeArray
    })
  },
  // 数据
  init() {
    ...
    const params = {
      ...
      areaId: this.id,
      // 城市rest
      cityRestful: cityRestfulDictionary,
      // 输入框
  ...
  setmap(fact) {
    ...
    this.map.centerAndZoom(sessionStorage.getItem("cityName"), 9.5);
          ...
          odiv[index].onclick = function () {
            ...
            that.$http.post('appServiceFarmhouse/mapcondition', params).then(res => {
                ...
                if (res.data.easyResult !== null) {
                  ...
                  if (res.data.easyResult.pageNum >= res.data.easyResult.page) {
                    if (res.data.easyResult.page == 1) {
                      that.easyResult = res.data.easyResult.rows
                      that.pageNum = res.data.easyResult.pageNum
                      that.page = res.data.easyResult.page
                      that.searchList = true
                    } else {
                      that.page++
                      that.easyResult = that.easyResult.concat(res.data.easyResult.rows)
                    }
                  }
                }
                that.setmap();
