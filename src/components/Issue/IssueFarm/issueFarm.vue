<template>
  <div style="margin-bottom: 1rem;">
    ...
    <div id="step1" v-show='step == 1'>
      <div class="list release">
        ...
        <div class="bot bot5">
          ...
          <div class="middle">
            <van-field type='number' v-model="coveredArea" @blur="generateTit" placeholder="请填写" />
          </div>
          <div class="right">
            <p>平方米</p>
          </div>
        ...
        <div class="bot bot6">
          ...
          <div class="middle">
            <van-field type='number' v-model="curtilageArea" @blur="generateTit" placeholder="请填写" />
          </div>
          <div class="right">
            <p>平方米</p>
          </div>
    ...
    <div id="step2" v-show='step == 2'>
      ...
      <div class="list release">
        <div class="bot">
          ...
          <div class="middle">
            <van-field type='number' v-model="floor" @blur="generateTit" placeholder="请填写" />
          </div>
          <div class="right">
            楼
          </div>
        ...
        <div class="bot">
          ...
          <div class="middle">
            <van-field type='number' v-model="propertyYear" @blur="generateTit" placeholder="请填写" />
          </div>
          <div class="right">
            年
          </div>
        ...
        <div class="bot">
          ...
          <div class="middle">
            <van-field v-model="builtYear" placeholder="请填写" />
          </div>
          <div class="right">
            年
            <div class="iconfont"></div>
        ...
        <div class="bot">
          ...
          <div class="middle">
            <van-field v-model="transformationYear" placeholder="请填写" />
          </div>
          <div class="right">
            年
            <div class="iconfont"></div>
      ...
      <div class="list release">
        ...
        <div class="bot">
          ...
          <div class="middle">
            <van-field v-model="phone" type="number" placeholder="请填写" />
          </div>
          <div class="right">
            <div class="iconfont"></div>
          </div>
    ...
    <van-popup v-model="popShow" position="bottom" :close-on-click-overlay="false" v-if="popType != 9"><!-- 新加上的，为了不显示位置标注的底部弹出层 -->
      ...
      <div class="once-columns" v-show="popType == 5">
        <van-picker :columns="houseOrientation" @change="houseOrientationChange"/>
      </div>
      <!-- 装修情况 -->
      <div class="once-columns" v-show="popType == 6">
        <van-picker :columns="decorationSituationColumns" @change="decorationSituationChange"/>
      </div>
      <!-- 房屋结构 -->
      <div class="once-columns" v-show="popType == 7">
        <van-picker :columns="buildingStructureColumns" @change="buildingStructureChange"/>
      </div>
      <!-- 房屋类型 -->
      <div class="once-columns" v-show="popType == 8">
        <van-picker :columns="houseTypeColumns" @change="houseTypeChange"/>
      </div>
      <!-- 地图 -->
      <!--<div v-if="popType == 9">
        <div class="map">
...
<script>
	...
	export default {
		...
		//实现此组件进入子路由时缓存（即“/locationAnnotation”对应的locationAnnotation.vue），退回父路由时销毁（即首页）
		beforeRouteLeave(to, from, next) {
			for(var index in this.$route.matched) {
				if(this.$route.matched[index].name == from.name) {
					if(to.name != "locationAnnotation") {
						if(sessionStorage.getItem("localPlace")) {
							sessionStorage.removeItem("localPlace");
						}
						if(sessionStorage.getItem("locationSpot")) {
							sessionStorage.removeItem("locationSpot");
						}
						//所有data初始化为原来的data
//						var allData = {
//							step: 1, // 第一页
...
//						this.init();
		...
		//由于此组件设置了meta的keepAlive来缓存。所以这里通过sessionStorage来获取路由“/locationAnnotation”传过来的参数
		beforeRouteEnter(to, from, next) {
			next(vm => {
				...
				if(sessionStorage.getItem("localPlace")) {
					vm.location = sessionStorage.getItem("localPlace");
				}
				if(sessionStorage.getItem("locationSpot")) {
					vm.locationSpot.lng = JSON.parse(sessionStorage.getItem("locationSpot")).lng;
					vm.locationSpot.lat = JSON.parse(sessionStorage.getItem("locationSpot")).lat;
				}
			});
		}
	}
