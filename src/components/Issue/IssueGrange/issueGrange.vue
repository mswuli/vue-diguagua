<template>
  <div class="issueGrange">
   ...
   <!--服务产品-->
    ...
    <van-popup v-model="popupShow" position="bottom" :close-on-click-overlay="false" v-if="popType != 9">
...
//						this.init();
		...
		beforeRouteEnter(to, from, next) {
			next(vm => {
				...
				if(sessionStorage.getItem("localPlace")) {
					vm.position = sessionStorage.getItem("localPlace");
				}
				if(sessionStorage.getItem("locationSpot")) {
    ...
    methods:{
      setmap(){
      	this.$router.push({ name: 'locationAnnotation' ,params:{location:this.position,locationSpot:this.locationSpot,columnsName:this.columnsName.toString().replace(/,/g,"-")}})
      	/*
        const _this=this;
      ...
      onChange(picker, value, index){//所在地区
      ...
      init(){
        ...
        this.$http.post('/appServiceInformation/publishGrangeParameter',params).then(res=>{
          if(res.msg=='success'){
            ...
            if(this.region[0].children){
              this.region[0].children.forEach((item,index)=>{
                this.columns1.push(item.label)
              })
            }
      ...
      confirmPop(){
        if(this.popType==1){ //所在地区
          if(this.columnsIndex1){ //街道
            let columnsName=[sessionStorage.getItem("cityName")],columnsIds=[sessionStorage.getItem("id")];
      ...
      submit(){//提交
        ...
        if(this.location !==''){
          params.areaName=this.location;
          params.areaId=this.columnsIds;
        }else{
          this.$toast("请选择所在地区")
          return false
        }
        ...
        console.log(params,'1',inn,'2',food)
       this.$http.post('/appServiceInformation/publishGrange',params).then(res=>{
         if(res.msg=='success'){
           this.$toast("发布成功")
           this.$router.push({name:'grangeindex'})
         }else{
           this.$toast(res.info)
         }
       })
    ...
    created:function(){
      if(localStorage.getItem('userInfo')){
        this.specialId=JSON.parse(localStorage.getItem('userInfo')).userId;
         this.teleNum=JSON.parse(localStorage.getItem('userInfo')).nickName;
        if(this.teleNum){
          this.businessTelephone=this.teleNum;
        }else{
          this.mobileValid=true;
        }
      }else{
